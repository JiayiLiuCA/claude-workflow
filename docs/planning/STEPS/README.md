...
- `STEP_NN_close.md` — 实录（Close 阶段产出：实际完成、偏离、关键决策、遗留、路线图校验、涉及的文件、从路线图剪切来的本 step 条目）

三者在 step close 后即为历史存档，不再修改；后续修正走 `OVERVIEW.md` 与域文件。`PROGRESS.md` 是这些实录的索引，也是已完成 step 的唯一入口（路线图只放未开工的 step）。
