...
不读整份 OVERVIEW，不读 PROGRESS 正文，不通读 diff。

# 第二步：分诊

根据第一步列出本次要改的地方，每处一行：哪个域文件的契约索引 / 表索引要加行、要写哪些行为参考；本 step 拍板的每条决议归哪里（只有一个域会引用 → 该域文件「决议」节；跨域 / 路线图级 / 待决 → OVERVIEW §3 台账）；是否新增 ADR；`.claude/rules/` 是否新增约定；路线图哪些条目可能受影响。
...
1. 域文件 `domains/<domain>.md`：契约索引 / 表索引各加一行（契约 + 用途 + 实现位置 + Step N）；**索引行只做定位**——本 step 改了既有契约时只在 Step 列追加号、必要时更新实现位置，不在行内写「(N) 改了什么」（改动过程在实录，行为口径写进行为参考）；单域决议落该域文件「决议」节（格式同台账，推翻用 ~~删除线~~ 保留原文）；行为参考只写代码读不出来的（行为表、edge case、集成改造点、实测结论、给后续 step 的承诺）。文件不存在则按 `domains/README.md` 的结构创建，并在 OVERVIEW §4 域索引加一行
2. OVERVIEW §3 决议台账：只落跨域 / 路线图级决议与待决项（单域决议已在上一条落域文件），格式 `- [x] 结论 + 理由（Step {N} 决议）`；推翻旧决议用 ~~删除线~~ 保留原文并写明新结论与原因，旧决议在台账还是域文件都照此办
3. ARCHITECTURE：仅当某个决策需固化为新 ADR（编号顺延，标注 Step {N} 落地）；新代码约定追加到对应的 `.claude/rules/<layer>.md`

# 第五步：路线图校验

对照本 step 的实况，逐条看 OVERVIEW §2 中后续 step 的条目（目标一句话 + 范围要点 + 依赖）是否仍然成立：依赖是否变了、范围是否该增减、顺序是否该调、是否有 step 该新增或废弃。
...
最后把本 step 的条目从 §2 **剪切**出来（§2 只留未开工的 step），末尾加 `✅ 完成于 {日期}`，留给第六步归档进实录。
...
## 实际完成
概括本 step 交付了什么。附测试通过数、关键 commit hash、分支名。
...
## 路线图校验
无变化，或：改了什么 + 为什么（详见 OVERVIEW 变更日志）。

## 涉及的文件
- 新建：…
- 修改：…
- 文档：更新的域文件与小节（如 `domains/ocr.md` §7.8、「决议」节）

## 路线图条目
（第五步从 OVERVIEW §2 剪切来的本 step 条目原文，含 ✅ 日期）
```

2. `PROGRESS.md`「Step 索引」表头下方插入一行：`| {N} | {标题} | {日期} | {一句话结论} | [实录](STEPS/STEP_{NN}_close.md) |`。一句话结论 = 后续 step 最需要知道的一条事实。这是已完成 step 的唯一索引（§2 已不再有它的条目）。
