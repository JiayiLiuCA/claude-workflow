...
根据第一步列出本次要改的地方，每处一行：哪个域文件的契约索引 / 表索引要加行、要写哪些行为参考；本 step 拍板的每条决议归哪里（只有一个域会引用 → 该域文件「决议」节；跨域 / 路线图级 / 待决 → PIPELINE §3 台账）；是否新增 ADR；`.claude/rules/` 是否新增约定；路线图哪些条目可能受影响。
...
1. 域文件 `pipeline/<domain>.md`：契约索引 / 表索引各加一行（契约 + 用途 + 实现位置 + Step N）；**索引行只做定位**——本 step 改了既有契约时只在 Step 列追加号、必要时更新实现位置，不在行内写「(N) 改了什么」（改动过程在实录，行为口径写进行为参考）；单域决议落该域文件「决议」节（格式同台账，推翻用 ~~删除线~~ 保留原文）；行为参考只写代码读不出来的（行为表、edge case、集成改造点、实测结论、给后续 step 的承诺）。文件不存在则按 `pipeline/README.md` 的结构创建，并在 PIPELINE §4 域索引加一行
2. PIPELINE §3 决议台账：只落跨域 / 路线图级决议与待决项（单域决议已在上一条落域文件），格式 `- [x] 结论 + 理由（Step {N} 决议）`；推翻旧决议用 ~~删除线~~ 保留原文并写明新结论与原因，旧决议在台账还是域文件都照此办
...
- 新建：…
- 修改：…
- 文档：更新的域文件与小节（如 `pipeline/ocr.md` §7.8、「决议」节）

## 路线图条目
（第五步从 PIPELINE §2 剪切来的本 step 条目原文，含 ✅ 日期）
```
