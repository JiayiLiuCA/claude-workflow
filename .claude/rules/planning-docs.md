...
**决议归属**：只有一个域会引用的决议记在该域文件「决议」节；跨域、架构级、路线图级决议与待决项记在 `PIPELINE.md` §3 台账。两处格式相同，都只增不删。
...
- `PIPELINE.md` 体量恒定：§2 只放未开工的 step（close 时条目剪切进实录、索引进 `PROGRESS.md`），§3 只放跨域决议与待决项；随 step 增长的内容进域文件或 `STEPS/`。
