...
**决议归属**：只有一个域会引用的决议记在该域文件「决议」节；跨域、架构级、路线图级决议与待决项记在 `OVERVIEW.md` §3 台账。两处格式相同，都只增不删。
...
- `STEPS/` 下的 discuss、plan、close 文件在 step close 后即为历史存档，不再修改；后续修正走 `OVERVIEW.md` 与域文件。
- ADR、决议台账与域文件「决议」节只增不删：被推翻的决策用 ~~删除线~~ 保留原文，写明推翻原因与新结论。错误结论也是资产。
- 路线图变更遵守 `OVERVIEW.md` §2 的规则：写变更日志、编号不变、废弃用删除线、受影响的 plan 标「需重跑」。
- `OVERVIEW.md` 体量恒定：§2 只放未开工的 step（close 时条目剪切进实录、索引进 `PROGRESS.md`），§3 只放跨域决议与待决项；随 step 增长的内容进域文件或 `STEPS/`。
- 域文件的契约索引 / 表索引行只做定位：后续改动只更新 Step 列与实现位置，不在行内累积「(N) 改了什么」的变更史——变更史在实录，行为口径在行为参考。
